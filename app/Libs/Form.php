...
ST;
            ...
            $block_html = <<<ST
                <div class="layui-form-item">
                    <label style="" class="layui-form-label">{$label_text}</label>
                    <div class="layui-input-block">
                        {$input_html}
                        $tip_html
                    </div>
...
ST;
    ...
    private function render_input($init_data, $value) {
        ...
        $init_data['verify'] = isset($init_data['verify']) ? $init_data['verify'] : '';
        $init_data['verify_msg'] = isset($init_data['verify_msg']) ? $init_data['verify_msg'] : '必填项不能为空';
...
str;
            ...
            foreach ($value as $ke => $val) {
                ...
                $html = <<<str
<input name="{$name_str}" lay-verify="{$init_data['verify']}" lay-reqtext="{$init_data['verify_msg']}"  value="{$val}" type="text"  class="layui-input"  {$disabled_str}/>
str;
            }
...
STR;
            ...
            $enum = join("\n", $enum);
            $html = <<<STR
            <select {$name_str} lay-verify="{$init_data['verify']}" lay-reqtext="{$init_data['verify_msg']}"  {$disabled_str} lay-search>
                <option value=""></option>
                {$enum}
            </select>
STR;
